    ...
    data () {
      return {
        ...
        dataForm: {
          ...
          logo: '',
          description: '',
          isshow: 1, // set default number
          firstLetter: '',
          sort: ''
        },
        dataRule: {
          name: [
            { required: true, message: 'Brand name cannot be empty', trigger: 'blur' }
          ],
          logo: [
            { required: true, message: 'Logo cannot be empty', trigger: 'blur' }
          ],
          description: [
            { required: true, message: 'description cannot be empty', trigger: 'blur' }
          ],
          isshow: [
            { required: true, message: 'Isshow cannot be empty', trigger: 'blur' }
          ],
          firstLetter: [
            // { required: true, message: 'First Letter cannot be empty', trigger: 'blur' }
            /**
             ...
             */
            {
              validator: (rule, value, callback) => {
                if (value == "") {
                  callback(new Error("First character cannot be empty"));
                } else if (!/^[a-zA-Z]$/.test(value)) {
                  callback(new Error("The first character must be within the range of a-z or A-Z."));
                } else {
                  callback();
                }
              }, trigger: 'blur' }
          ...
          sort: [
            ...
            {validator: (rule, value, callback) => {
              if (value == "") {
                ...
                callback(new Error("Sort number cannot be less than 0"));
              } else {
                callback();
              }
            }, trigger: 'blur'}

<template>
  <div>
    ...
    <el-tree
      ref="categoryTree" :data="data" :default-expanded-keys="expandedKey" :props="defaultProps"
             :expand-on-click-node="false" show-checkbox node-key="id">
      <!-- (增加的属性，方法等，都可以参考 elementUI Tree 树形控件 的文档说明)
      1. 使用 vue 的插槽机制在每个菜单后加入 Append 和 Delete 连接
      2. slot-scope="{ node, data } : 是一种解构写法, node 表示当前节点, data
      就是从数据库取出的分类对象信息
      3. @node-click="handleNodeClick" 没有什么用，删除，注意把对应的 method也删除
      4. 增加相应的 append(data) 和 remove(node, data) 方法
      5. :expand-on-click-node="false": 设置 false 表示点击 Append 和 Delete 连接 不展开下级分类
    ...
    <!-- :visible.sync="dialogVisible" 动态绑定 dialogVisible ,
    如果为 true,就显示对话框，否则不显示-->
    <el-dialog title="添加分类" :visible.sync="dialogVisible" width="30%">
    <!-- 动态绑定 category 数据对象, category 在数据池定义-->
...
export default {
  data() {
    return {
      ...
      category: { //初始化添加分类的表单对象数据
        name: "", parentId: 0, catLevel: 0,
        isShow: 1, sort: 0, proCount: 0, proUnit: "",
        icon: "",
        id: null
      },
      data: [],
  ...
  methods: {
    ...
    edit(data) {
      this.dialogType = "edit"; //设置对话框类型为 edit
    ...
    append(data) {
      this.dialogType = "add"; //设置对话框类型为 add
      this.dialogVisible = true;
    ...
    updateCategory() {
       ...
       * 3. 这里我们只是发送了要修改的字段信息，没有发送的字段在数据库不会被修改(保持原值, 可以通过后台 update 语句看)
       */
    ...
    getCategories() {
      this.$http({
        ...
        /*.then((data) => { // 这是最原始的写法,好理解，但是有的难看, 这里的 data 就是请求返回的数据

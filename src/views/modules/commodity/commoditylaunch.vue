        ...
        <el-card class="box-card" style="width:80%;margin:20px auto">
          <el-form ref="spuBaseForm" :model="spu" label-width="120px" :rules="spuBaseInfoRules">
            ...
            <el-form-item label="选择分类" prop="categoryId">
              ...
              <el-cascader :cascadedCategoryId.sync="cascadedCategoryId" v-model="cascadedCategoryId" :options="categories" :props="props"></el-cascader>
            </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      uploadImages: [],
      step: 0,
      //spu_name  spu_description  category_id  brand_id  weight  publish_status
      spu: {
        //要提交的数据
        spuName: "",
        spuDescription: "",
  ...
  watch: {
    ...
    cascadedCategoryId(path) {
      ...
      //把当前的分类id，也赋给this.spu.categoryId , 后面在请求分类下的属性组和属性有用
      this.spu.categoryId = this.categoryId;
      this.getCatBrands();
    },
  ...
  methods: {
    ...
    getShowSaleAttr() {
      console.log("this.spu.categoryId=", this.spu.categoryId)
      //获取当前分类可以使用的销售属性
      if (!this.dataResp.steped[1]) {
        this.$http({
          url: this.$http.adornUrl(`/commodity/attr/sale/list/${this.spu.categoryId}`),
          method: "get",
          params: this.$http.adornParams({
            page: 1,
            limit: 500
    ...
    showBaseAttrs() {//得到当前分类的基本属性
      if (!this.dataResp.steped[0]) {
        this.$http({
          url: this.$http.adornUrl(`/commodity/attrgroup/${this.spu.categoryId}/withattr`),
          method: "get",
          params: this.$http.adornParams({})
        }).then(({data}) => {
          console.log("data-->", data)
